...
public class SpecialtyServiceImpl implements ISpecialtyService {
    ...
    public void deleteById(Long id) throws ResourceNotFoundException, DuplicateResourceException {
        ...
        if (specialty.getDentists() != null && !specialty.getDentists().isEmpty()) {
            throw new DuplicateResourceException(
                    "No se puede eliminar la especialidad porque está asignada a " + specialty.getDentists().size() + " dentista(s)");
        }
    ...
    @Override
    @Transactional
    public void removeSpecialtyFromDentist(Long dentistId, Long specialtyId) throws ResourceNotFoundException {
        Dentist dentist = dentistRepository.findById(dentistId)
                .orElseThrow(() -> new ResourceNotFoundException("Dentista no encontrado con id: " + dentistId));
        Specialty specialty = specialtyRepository.findById(specialtyId)
                .orElseThrow(() -> new ResourceNotFoundException("Especialidad no encontrada con id: " + specialtyId));
