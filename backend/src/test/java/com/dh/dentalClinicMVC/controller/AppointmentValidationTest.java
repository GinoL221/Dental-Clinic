...
public class AppointmentValidationTest {
    ...
    public void createAppointmentWithPastTimeTodayShouldReturnBadRequest() throws Exception {
        ...
        boolean isWeekend = today.getDayOfWeek() == java.time.DayOfWeek.SATURDAY || today.getDayOfWeek() == java.time.DayOfWeek.SUNDAY;
        String expectedMessage = isWeekend
    ...
    public void createAppointmentOutsideWorkingHoursShouldReturnBadRequest() throws Exception {
        // Crear dentista
        String dentistJson = "{\"registrationNumber\":7755,\"firstName\":\"Al\",\"lastName\":\"Bo\",\"email\":\"v5dentist@example.com\"}";
        String dentistResponse = mockMvc.perform(post("/dentists").with(csrf()).contentType(MediaType.APPLICATION_JSON).content(dentistJson))
                .andExpect(status().isCreated())
                .andReturn().getResponse().getContentAsString();
        int dentistId = objectMapper.readTree(dentistResponse).get("id").asInt();
        ...
        // Crear paciente
        String admissionDate = LocalDate.now().toString();
        String patientJson = String.format("{\"cardIdentity\":7755,\"firstName\":\"Pa\",\"lastName\":\"Qu\",\"email\":\"v5patient@example.com\",\"admissionDate\":\"%s\"}", admissionDate);
        String patientResponse = mockMvc.perform(post("/patients").with(csrf()).contentType(MediaType.APPLICATION_JSON).content(patientJson))
                .andExpect(status().isCreated())
                .andReturn().getResponse().getContentAsString();
        int patientId = objectMapper.readTree(patientResponse).get("id").asInt();
        ...
        while (weekday.getDayOfWeek() == java.time.DayOfWeek.SATURDAY || weekday.getDayOfWeek() == java.time.DayOfWeek.SUNDAY) {
            weekday = weekday.plusDays(1);
        }
        String dateStr = weekday.toString();

        // Probar 07:59 -> 400
        AppointmentRequestDTO app0759 = new AppointmentRequestDTO(
                (long) dentistId,
                (long) patientId,
                dateStr,
                "07:59",
                "Outside working hours 07:59"
        );
        ...
        // Probar 18:01 -> 400
        AppointmentRequestDTO app1801 = new AppointmentRequestDTO(
                (long) dentistId,
                (long) patientId,
                dateStr,
                "18:01",
                "Outside working hours 18:01"
        );
        mockMvc.perform(post("/appointments").with(csrf())
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    public void createAppointmentWithinWorkingHoursShouldSucceed() throws Exception {
        // Crear dentista
        String dentistJson = "{\"registrationNumber\":7756,\"firstName\":\"Al\",\"lastName\":\"Bo\",\"email\":\"v6dentist@example.com\"}";
        String dentistResponse = mockMvc.perform(post("/dentists").with(csrf()).contentType(MediaType.APPLICATION_JSON).content(dentistJson))
                .andExpect(status().isCreated())
                .andReturn().getResponse().getContentAsString();
        int dentistId = objectMapper.readTree(dentistResponse).get("id").asInt();
        ...
        // Crear paciente
        String admissionDate = LocalDate.now().toString();
        String patientJson = String.format("{\"cardIdentity\":7756,\"firstName\":\"Pa\",\"lastName\":\"Qu\",\"email\":\"v6patient@example.com\",\"admissionDate\":\"%s\"}", admissionDate);
        String patientResponse = mockMvc.perform(post("/patients").with(csrf()).contentType(MediaType.APPLICATION_JSON).content(patientJson))
                .andExpect(status().isCreated())
                .andReturn().getResponse().getContentAsString();
        int patientId = objectMapper.readTree(patientResponse).get("id").asInt();
        ...
        // Probar 08:00 -> 200
        AppointmentRequestDTO app0800 = new AppointmentRequestDTO(
                (long) dentistId,
                (long) patientId,
                dateStr,
                "08:00",
        ...
        // Probar 18:00 -> 200
        AppointmentRequestDTO app1800 = new AppointmentRequestDTO(
                (long) dentistId,
                (long) patientId,
                dateStr,
                "18:00",
    ...
    public void createAppointmentOnWeekendShouldReturnBadRequest() throws Exception {
        // Crear dentista
        String dentistJson = "{\"registrationNumber\":7757,\"firstName\":\"Al\",\"lastName\":\"Bo\",\"email\":\"v7dentist@example.com\"}";
        String dentistResponse = mockMvc.perform(post("/dentists").with(csrf()).contentType(MediaType.APPLICATION_JSON).content(dentistJson))
                .andExpect(status().isCreated())
                .andReturn().getResponse().getContentAsString();
        int dentistId = objectMapper.readTree(dentistResponse).get("id").asInt();
        ...
        // Crear paciente
        String admissionDate = LocalDate.now().toString();
        String patientJson = String.format("{\"cardIdentity\":7757,\"firstName\":\"Pa\",\"lastName\":\"Qu\",\"email\":\"v7patient@example.com\",\"admissionDate\":\"%s\"}", admissionDate);
        String patientResponse = mockMvc.perform(post("/patients").with(csrf()).contentType(MediaType.APPLICATION_JSON).content(patientJson))
                .andExpect(status().isCreated())
                .andReturn().getResponse().getContentAsString();
        int patientId = objectMapper.readTree(patientResponse).get("id").asInt();
        ...
        // Probar Sábado -> 400
        AppointmentRequestDTO appSaturday = new AppointmentRequestDTO(
                (long) dentistId,
                (long) patientId,
        ...
        mockMvc.perform(post("/appointments").with(csrf())
                ...
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.message").value("Solo se pueden programar citas de lunes a viernes"));

        // Probar Domingo -> 400
        AppointmentRequestDTO appSunday = new AppointmentRequestDTO(
                (long) dentistId,
                (long) patientId,
                nextSunday.toString(),
                "10:00",
