...
class SpecialtyControllerTest {
    ...
    void testGetSpecialtyById() throws Exception {
        mockMvc.perform(get("/specialties/{id}", savedSpecialty.getId()))
                .andExpect(status().isOk())
    ...
    void testGetSpecialtyById_NotFound() throws Exception {
        mockMvc.perform(get("/specialties/{id}", 9999))
    ...
    void testUpdateSpecialty() throws Exception {
        String json = "{\"name\":\"Odontología General Actualizada\",\"description\":\"Descripción actualizada\"}";

        mockMvc.perform(put("/specialties/{id}", savedSpecialty.getId())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(json))
                .andExpect(status().isOk())
    ...
    void testDeleteSpecialty() throws Exception {
        mockMvc.perform(delete("/specialties/{id}", savedSpecialty.getId()))
    ...
    void testDeleteSpecialty_NotFound() throws Exception {
        mockMvc.perform(delete("/specialties/{id}", 9999))
                .andExpect(status().isNotFound());
    }
}