...
- [x] 5.1 Run `cd frontend && npm test` once more on the fully-migrated tree — full suite green, guard green. **Verified by sdd-verify (2026-06-21)**: 178 passed, 6 skipped, 0 failed (fresh run, not cached).
- [x] 5.2 Live-verify spot-check across all 4 entities in one pass (list/add/edit pages + login + a non-auth page) to confirm no cross-entity interaction regressed after all slices combined. **Verified by sdd-verify**: continuous cross-entity smoke test (single login, sequential visits to all 4 entities + return to non-auth page) — zero accumulated console/page errors, all controllers initialized exactly once per page.
- [x] 5.3 Confirm proposal's Success Criteria checklist is fully satisfied (guard exists+passes+fails-on-reintroduction; each entity exports init with no self-running duplicate listener except Auth's single routed listener; no `window.*` global added/removed/renamed/behavior-changed; Auth initializes on both entry points). **Verified by sdd-verify**: all success criteria independently confirmed via source inspection + live smoke test; VERIFY.md section "Spec Conformance" re-derived all 4 requirements without trusting apply-progress.
