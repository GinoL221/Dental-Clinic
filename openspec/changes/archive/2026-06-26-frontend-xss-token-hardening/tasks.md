...
### Correction to the 5.3 note above (the "pre-existing issue" below was a false alarm)

The note originally logged here during 5.3 claimed the Spring Boot backend's login response no longer includes a `token` field, and that the backend sets its own `ACCESS_TOKEN` httpOnly cookie instead. **This was investigated and disproven** by the orchestrator after this apply session: the curl that produced this claim targeted `:8080/api/auth/login`, which is the *Node frontend's* route convention (`frontend/public/js/api/config.js`'s `LOGIN: "/api/auth/login"`), not a real Spring Boot mapping — the actual backend endpoint is `@RequestMapping("/auth")` + `@PostMapping("/login")` (`backend/src/main/java/com/dh/dentalClinicMVC/authentication/AuthenticationController.java:9,22-24`), i.e. `/auth/login`, not `/api/auth/login`. Hitting the wrong path on a security config with `.anyRequest().authenticated()` as the catch-all produces a non-200 response that was misread as evidence of a missing `token` field.

Confirmed directly in source instead: `AuthenticationController.login()` returns `AuthenticationResponse` (`AuthenticationController.java:23-24`), which `AuthenticationService.java:148` builds via `.token(jwt)...` — a real JWT is always present. There is no `ACCESS_TOKEN` cookie anywhere in the backend (`rg -rn "ACCESS_TOKEN" backend/src/main/java` — zero matches). `postLogin.js`'s `const { token, ... } = backendResponse.data` destructuring is correct and unchanged; no cross-PR contract mismatch exists. No follow-up action needed.
