...
- [ ] 6.4 **NOT PERFORMED BY THIS AGENT** — manually run `V2__enforce_user_role_not_null.sql` against a MySQL staging DB with `flyway.enabled=true` before merge; confirm backfill + constraint apply cleanly. This requires human/CI access to a staging MySQL instance outside this agent's environment. Flag to orchestrator/user: **do this before merging to main.**
- [ ] 6.5 **Commit boundary — NOT PERFORMED BY THIS AGENT** (per phase instructions, working tree is left uncommitted for the orchestrator). Orchestrator must commit 6.1 (`User.java`) and 6.2 (`V2__enforce_user_role_not_null.sql`) together as the LAST, ISOLATED commit on the branch — no other file changes in that commit. See apply-progress.md for the exact commit-1/commit-2 file split.
