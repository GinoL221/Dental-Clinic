...
MUST NOT be a route to ADMIN, regardless of which exact mechanism (`sdd-design`) is used to close
that path (reject vs. silently downgrade vs. restrict the public `role` enum).
