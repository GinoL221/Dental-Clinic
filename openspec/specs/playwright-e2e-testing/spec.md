...
## Purpose
Validate complete user journeys using a real browser, with mock and full-stack evidence kept explicitly separate so mock results can never masquerade as proof of real backend behavior.
...
This spec does not require Firefox or WebKit, a complete role matrix, the full booking lifecycle, or any new use of the obsolete `/api/auth/validate` endpoint. Portfolio screenshots remain unless proven Playwright-generated.
