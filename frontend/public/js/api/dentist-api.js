...
const DentistAPI = {
    ...
    async getByRegistrationNumber(registrationNumber) {
        try {
            ...
            const response = await fetch(`${API_BASE_URL}/dentists/registration/${registrationNumber}`, {
                method: 'GET',
                headers: getAuthHeaders()
            });
    ...
    validateDentistData(dentist, isUpdate = false) {
        ...
        if (!dentist.registrationNumber || dentist.registrationNumber.trim().length < 3) {
