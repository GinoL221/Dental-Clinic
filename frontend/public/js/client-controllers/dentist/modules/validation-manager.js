class DentistValidationManager {
  ...
  validateDentistData(data) {
    ...
    const requiredFields = ["name", "lastName", "registrationNumber"];
  ...
  async validateUniqueRegistrationNumber(
    ...
    try {
      ...
      if (!registrationNumber || typeof registrationNumber !== "string") {
        return { isValid: true, message: "" }; // No validar si no hay valor
      }
      ...
      if (duplicate) {
        return {
          isValid: false,
          message: `La matrícula ${registrationNumber} ya está registrada para Dr. ${duplicate.name} ${duplicate.lastName}`,
        };
      }
  ...
  getFieldNameFromElement(element) {
    ...
    if (element.id) {
      if (element.id === "firstName") return "name";
      if (this.validationRules[element.id]) return element.id;
    }
