...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    // Inputs de filtro
    const searchPatient = /** @type {HTMLInputElement | null} */ (
      document.getElementById('searchPatient')
    );
    const filterDentist = /** @type {HTMLInputElement | null} */ (
      document.getElementById('filterDentist')
    );
    const filterDate = /** @type {HTMLInputElement | null} */ (
      ...
      searchPatient.addEventListener('input', function () {
        ...
        matches.slice(0, 5).forEach((patient) => {
          ...
          item.addEventListener('click', () => {
            searchPatient.value = `${patient.firstName || ''} ${patient.lastName || ''}`;
            const selectedPatientId = /** @type {HTMLInputElement | null} */ (
    ...
    if (dentistInput) {
      dentistInput.addEventListener('input', function () {
        ...
        matches.slice(0, 5).forEach((dentist) => {
          ...
          item.addEventListener('click', () => {
            dentistInput.value = `${dentist.firstName || ''} ${dentist.lastName || ''}`;
            const selectedDentistId = /** @type {HTMLInputElement | null} */ (
              document.getElementById('selectedDentistId')
            );
    ...
    function applyFilters() {
      const selectedPatientIdInput = /** @type {HTMLInputElement | null} */ (
        document.getElementById('selectedPatientId')
      );
      const selectedDentistIdInput = /** @type {HTMLInputElement | null} */ (
        document.getElementById('selectedDentistId')
      );
      const filterStatusSelect = /** @type {HTMLSelectElement | null} */ (
    ...
    function clearFilters() {
      ...
      const filterStatus = /** @type {HTMLSelectElement | null} */ (
        document.getElementById('filterStatus')
      );
    ...
    showErrorMessage('Error al cargar la lista de citas. Por favor, recargue la página.');
  }
});
...
function setupGlobalFunctions() {
  ...
  window.sortAppointments = function (
    /** @type {string} */ sortBy,
  ...
  window.paginateAppointments = function (
    /** @type {number} */ page,
    /** @type {number} */ limit = 10,
  ) {
...
window.debugAppointmentListController = function () {
  return {
    ...
    listState: appointmentController ? appointmentController.getListState() : null,
    modulesAvailable: {
    ...
    globalFunctions: {
      ...
      refreshAppointmentsList: typeof window.refreshAppointmentsList === 'function',
      ...
      getSelectedAppointments: typeof window.getSelectedAppointments === 'function',
      bulkDeleteAppointments: typeof window.bulkDeleteAppointments === 'function',
