...
describe('PatientUIManager.createPatientTableRow — XSS safe-DOM rendering', () => {
  ...
  test('a markup-free patient still renders the expected visible text and action buttons', () => {
    ...
    expect(row.querySelector('.patient-full-name').textContent).toBe('Carlos Diaz');
    expect(row.querySelector('.patient-email-text').textContent).toBe('carlos.diaz@example.com');
    expect(row.querySelector('.patient-id').textContent).toBe('3');

    const editButton = row.querySelector('button[onclick="editPatient(3)"]');
    const deleteButton = row.querySelector('button[onclick="deletePatient(3)"]');
