...
const ENTITY_FILE_SETS = {
  ...
  dentist: {
    ...
    wrappers: ['dentist/dentist-controller.js', 'dentist/dentist-list-controller.js'],
...
const EXCLUDED_FILES = ['dentist/dentist-specialty-ui.js', 'dashboard/dashboard-controller.js'];
...
describe('global-wiring-source guard: singleton exemption', () => {
  test('dentistController is assigned by the canonical and the exemption rule covers it even if a wrapper also assigned it', () => {
    ...
    expect(extractWindowAssignments(canonical).has('dentistController')).toBe(true);
...
describe('global-wiring-source guard: actionable failure message shape', () => {
  ...
  test('failure message names the offending global and both file paths', () => {
    const fileSources = {
      ...
      'dentist/dentist-list-controller.js': 'window.searchDentists = function (term) {};',
    };
  ...
  test('does not report a violation for the expected singleton duplication', () => {
    const fileSources = {
      'dentist/modules/index.js': 'window.dentistController = new DentistController();',
      'dentist/dentist-controller.js': 'window.dentistController = dentistController;',
    };
